# TOOL hfst-subtract.py: "Difference"
# (Compute the difference of two HFST transducers. Match archives pairwise, or broadcast the second archive.)
...
names.output('output.hfst', names.replace('input1.hfst', '-d.hfst'))
...
with Popen(['hfst-subtract', '-o', 'output.hfst']
           + symbolharm
           + flagharm
...
hfst.finish(require = 'output.hfst',
            version = 'hfst-subtract' if VersionLog == 'produce' else None)
