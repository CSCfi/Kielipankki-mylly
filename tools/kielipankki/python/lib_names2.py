...
# Usage in a Mylly tool script: enforce that an input display name matches a
# pattern, and base output display names on their first components:
#
...
#   maps output.ext => ensin.1.svg in chipster-outputs.tsv
...
def base(argname, pattern, *, cash = None):
    '''Enforces that there is a corresponding input
    name in chipster_inputs.tsv, it matches the
    ...
    if fnmatch(name, pattern):
        ...
        print('does not match:', pattern, file = sys.stderr)
        exit(1)
