...
# TODO: These functions need to receive wrapname from the tool. Like
# the new restore_inputs(wrapname, tag) already does. Done now?
...
# ./<wrapname> (the same directory will do just fine); unzip -l
# <wrapname> to see where in your $WRKDIR the batch job is working.
#
# Use the wrapname in the tool header.
#
...
def setup_wrap(wrapname, tag, *datadata):
    '''
    Wraps ./text.txt in wrapname (e.g. ./data.job) as ./data/text.txt
    (note: wrapname is meant to be just a basename, no path, or ./ is ok)

    Creates $WRKDIR/wrap<tmp>, outside chipster control!

    ./data.job (wrapname) is a zip archive file containing the
    following two items:
    ...
    with ZipFile(wrapname, "w") as wrap:
...
def setup_job(wrapname, tag, template):
    ...
    # should really check that ./<wrapname> is there and is a zip file
...
'''
    First time td-job (another "tool") on ./<wrapname>
    - checks ./<wrapname> claims to be a "Turku Dependency Wrap"
    ...
    Subsequent times td-job on ./<wrapname>
    - produces ./status.log from the queue (wait, which queue? which job?)

    When td-job on ./<wrapname> finds the job finished and result there, it
...
def process_wrap(wrapname, tag, *results):
    '''
    Process ./<wrapname> as follows:

    Extract <name> from ticket/<name> in ./<wrapname>, check that the
    member has tag (e.g. "Turku Dependency Wrap") on its first line.

    Try to create $WRKDIR/<name>/state/.
    
    - success -> extract data/* from ./<wrapname> to
    ...
    try:
        ...
        with open("status.log", "a") as log:
            print("Job directory for this job file was not found",
                  "",
                  "This is normal after a final status is reported,",
                  "at which time any results should have appeared.",
                  "",
                  "If an unfinished job file is old, prepare the data again.",
                  sep = '\n',
                  file = log)
...
def submit_job(work, results):
    '''
    <work>/state/ is empty
    <work>/data/<whatever> is there (just extracted from ./<wrapname>)
    <work>/wrap.job should be there (ever since ./<wrapname> was made)
