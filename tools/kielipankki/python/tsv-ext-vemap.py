# TOOL tsv-ext-vemap.py: "Extend from key=value|..."
# (Extend each record from a key-value attribute. A non-empty map contains key=value pairs separated by vertical bars, with no duplicate keys. The default is to extend with those keys that occur.)
